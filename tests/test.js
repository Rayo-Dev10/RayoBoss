...
const storageFactory = require('../server/core/storage/storageFactory');
const StorageProvider = require('../server/core/storage/StorageProvider');
...
(async () => {
  ...
  await test('1. Health informa versión 4.0.0', async () => {
    const result = await request('GET', '/api/health');
    assert.equal(result.status, 200);
    assert.equal(result.body.version, '4.0.0');
  });
  ...
  await test('58. Ruta multimedia delega almacenamiento y no importa proveedores concretos', async () => {
    ...
    assert.ok(routeSource.includes("require('../core/storage/storageFactory')"));
    assert.ok(!routeSource.includes("@vercel/blob"));
    assert.ok(!routeSource.includes('fs.renameSync'));
    assert.ok(fs.existsSync(path.join(root, 'server/core/storage/LocalDiskStorageProvider.js')));
    assert.ok(fs.existsSync(path.join(root, 'server/core/storage/VercelBlobStorageProvider.js')));
  });
  ...
  await test('60. Vercel sin Blob despliega en modo demostración de solo lectura', async () => {
    ...
    const script = "const p=require('./server/core/storage/storageFactory').getStorageProvider();process.stdout.write(JSON.stringify(p.describe()))";
  ...
  console.log('\nRayoBoss 4.0.0 - Reporte de pruebas');
